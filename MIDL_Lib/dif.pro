Function Dif, vec, ord, backward = back, centered = cent, forward = forw, $
...
;-
	...
	if Isnum(vec,type=typ) and (size(vec))(0) le 1 then n = n_elements(vec) $
	...
	if nord gt 0 then begin
		case One_of(edx,ldx) of
			-1:	res = [make_array(nord,type=typ),vec,make_array(nord,type=typ)]
			0 :	res = [replicate(vec[0],nord),vec,replicate(vec[n-1],nord)]
			1 :	begin
					dum = Cast(findgen(nord),typ,typ)
					bot = (vec[1<(n-1)] - vec[0])*(dum-nord) + vec[0]
					top = (vec[n-1] - vec[(n-2)>0])*(dum+1) + vec[n-1]
					res = [bot,vec,top]
				end
		endcase
