Pro Tabulate, a, b, c, d, e, f, g, h, index = ind, from = fro, to = to, $
	...
	file = fnam, first_ind = fnd, IDF_off = idf, more = mor, _extra = _e
...
;		default is .TXT).  If provided, the output is sent to this file,
...
;				FILE_GET.
;	FIRST_IND
;		Starting value for the index counter.  Effective only when /INDEX is
;		set.  Defaults to 0.
;	/IDF_OFF
...
;	_EXTRA
;		A formal keyword, used to pass keywords to FILE_GET.  Not to be used
;		directly.
; OUTPUTS:
...
; PROCEDURE:
;		Straightforward.  Uses ABS_MM, DEFAULT, FILE_GET, STREQ, STRPARSE_MM
;		and TYPE from MIDL.
...
;		Modified 25-NOV-2004 by Mati Meron.  Added keyword FIRST_IND.
;		Modified 15-NOV-2005 by Mati Meron.  Improved file writing options.
;-
	...
	if infl then begin
		n = lindgen(nro) + Default(fnd,0l,/dtyp)
		if nro le 2^15-1 then n = fix(n)
		inwd = (floor(alog10(max(n)>1)) + 1)
	endif
	...
	pf = fdef[cinf]
	pw = wdef[cinf]
	if infl then pw[0] = inwd > wmin[cinf[0]]
	pd = ddef[cinf]
	...
	if filfl then begin
		wnam = File_get(fnam,default='.txt',/write,/over,stat=stat,_extra=_e)
		if stat then begin
		...
		if keyword_set(mor) then begin
			if not Streq(!version.os,'win',3) then clear_screen else print
